...
const player1TypeInput = document.getElementById('player1-type');
const player1UrlInput = document.getElementById('player1-url');
const player1UrlGroup = document.getElementById('player1-url-group');
const player1NameInput = document.getElementById('player1-name');
const player1ColorInput = document.getElementById('player1-color');
const player2TypeInput = document.getElementById('player2-type');
const player2UrlInput = document.getElementById('player2-url');
const player2UrlGroup = document.getElementById('player2-url-group');
...
function saveConfig() {
    config.player1.type = player1TypeInput.value;
    ...
    config.player2.type = player2TypeInput.value;
...
async function handleCellClick(column) {
    try {
        ...
        const playerConfig = currentPlayer === 1 ? config.player1 : config.player2;
        ...
        if (!gameState.valid_moves.includes(column)) {
            return; // Invalid move
        }
        ...
        if (!response.ok) {
            ...
            console.error('Human move failed:', error.error);
            return;
        }
...
function updatePlayerColors() {
    ...
    const oldStyle = document.getElementById('player-colors-style');
    if (oldStyle) {
        oldStyle.remove();
    }
...
function updateGameStatus(state) {
    if (state.game_over) {
        ...
        const playerConfig = state.current_player === 1 ? config.player1 : config.player2;
        const turnText = playerConfig.type === 'human' ? `${currentPlayerName}'s turn (click to move)` : `${currentPlayerName}'s turn`;
        gameStatus.textContent = turnText;
        gameStatus.style.color = '#333';
    }
...
async function startGame() {
    try {
        ...
        // Only start AI game loop if both players are AI
        const bothAI = config.player1.type === 'ai' && config.player2.type === 'ai';
        ...
        if (bothAI) {
            ...
            // For games with human players, reset the game state
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    player1TypeInput.addEventListener('change', (e) => {
        const isHuman = e.target.value === 'human';
        player1UrlGroup.style.display = isHuman ? 'none' : 'block';
        player1NameInput.value = isHuman ? 'Human Player' : 'Random AI';
        config.player1.type = e.target.value;
    });

    player2TypeInput.addEventListener('change', (e) => {
        const isHuman = e.target.value === 'human';
        player2UrlGroup.style.display = isHuman ? 'none' : 'block';
        player2NameInput.value = isHuman ? 'Human Player' : 'Perfect AI';
        config.player2.type = e.target.value;
    });

    // Initialize URL group visibility
    player1UrlGroup.style.display = player1TypeInput.value === 'human' ? 'none' : 'block';
    player2UrlGroup.style.display = player2TypeInput.value === 'human' ? 'none' : 'block';
